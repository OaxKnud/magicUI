<template>
    <!-- Tooltip 组件的根容器 -->
    <div class="mg-tooltip" ref="popperContainerNode" v-on="outerEvents">
        <!-- 触发 Tooltip 显示的元素 -->
        <div class="mg-tooltip__trigger" ref="triggerNode" v-on="events">
            <!-- 插槽内容 -->
            <slot />
        </div>
        <!-- Tooltip 弹出框的过渡效果 -->
        <Transition :name="transition">
            ...
            <div v-if="isOpen" class="mg-tooltip__popper" ref="popperNode" v-on="dropdownEvents">
                <!-- 内容插槽 -->
                <slot name="content">
                    {{ content }}
                </slot>
