 ...
 */
<template lang="">
  <div class="mg-input" :class="computedClass">
    <!-- label -->
    <label v-if="label" class="mg-input-label">
      {{ label }}
    </label>
    <!-- Prepend slot 前缀slot -->
    <div v-if="$slots.prepend" class="mg-input-prepend">
      <slot name="prepend"/>
    </div>
    <!-- 渲染Input -->
    <div class="mg-input-container">
      <!-- Prefix Slot 前缀图标 -->
      <span v-if="$slots.prefix" class="mg-input-prefix">
        <slot name="prefix"/>
      </span>
      <input
        ref="inputInstance"
        v-model="inputValue"
        class="mg-input-content"
        :type="inputType"
        :disabled="disabled"
      ...
      <span 
        ...
        @click="clearInput"
        class="mg-input-clear"
      >
        <mg-icon icon="circle-xmark"></mg-icon>
      </span>
      <!-- eye 图标 显示密码 -->
      <span 
        v-show="isEyeShow" 
        ...
        class="mg-input-eye"
      >
        <mg-icon :icon="eyeIcon"></mg-icon>
      </span>
      <!-- Suffix Slot 后缀图标 -->
      <span v-if="showSuffixIcon" class="mg-input-suffix">
        <slot name="suffix"/>
      </span>
    </div>
    <!-- Prepend slot 后缀slot -->
    <div v-if="$slots.append" class="mg-input-append">
      <slot name="append"/>
    </div>
  </div>
</template>
...
const computedClass = computed(() => {
  return {
    [`mg-input--${props.type}`]: props.type,
    'is-disabled': props.disabled,
    'is-focus': isFocus,
    [`mg-input--${props.size}`]: props.size
  }
})
